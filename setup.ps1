...
$BtnPresetKitAndyz0x.Add_Click({
    ...
    Write-GuiLog "Selecionados: Brave, VLC, ShareX, Notepad++, Antigravity IDE, Discord, Steam, Epic Games, WhatsApp, NodeJS LTS, Git, Python 3, PDF24, NanaZip, IDM, Nilesoft Shell, Proton Pass, Parsec, Snappy Driver, .NET Runtimes (6, 8 e 9), AutoRuns e Visual C++ (32 e 64 bits)." "INFO"
})
                        ...
                        <ControlTemplate TargetType="Button">
                            <Border Name="Brd" Background="{TemplateBinding Background}" BorderBrush="{TemplateBinding BorderBrush}" BorderThickness="{TemplateBinding BorderThickness}" CornerRadius="8" Padding="{TemplateBinding Padding}">
                                <StackPanel>
                                    ...
                                    <TextBlock Text="Marca: NVIDIA App (Drivers &amp; Otimização) + NVCleanstall + Utilitários Gamer" FontSize="11" Foreground="#A7F3D0" Margin="0,3,0,0" TextWrapping="Wrap"/>
                                </StackPanel>
                            </Border>
                            <ControlTemplate.Triggers>
                                <Trigger Property="IsMouseOver" Value="True">
...
$BtnPresetPackGamer.Add_Click({
    ...
    if ($gpuChoice -eq "NVIDIA") {
        ...
        if ($Global:AppCheckBoxes.ContainsKey("WPFInstallnvclean")) {
            $Global:AppCheckBoxes["WPFInstallnvclean"].CheckBox.IsChecked = $true
        }
        ...
        Write-GuiLog "🔥 Pack PC Gamer ativado com perfil NVIDIA! (NVIDIA App + NVCleanstall selecionados)" "SUCCESS"
...
$BtnPresetGamer.Add_Click({
    foreach ($item in $Global:AppCheckBoxes.Values) { $item.CheckBox.IsChecked = $false }
    $gamers = @("WPFInstallchrome", "WPFInstall7zip", "WPFInstalldiscord", "WPFInstallspotify", "WPFInstallsteam", "WPFInstallepicgameslauncher")
    foreach ($k in $gamers) {
        if ($Global:AppCheckBoxes.ContainsKey($k)) {
            $Global:AppCheckBoxes[$k].CheckBox.IsChecked = $true
        }
