...
.DESCRIPTION
    - 264 Aplicativos organizados com Ícones Oficiais e indicador nítido de Open Source (FOSS).
...
Write-Host "    264 Aplicativos • Pack PC Gamer • Otimizações & Tweaks" -ForegroundColor DarkGray
Write-Host "  ==================================================================" -ForegroundColor Cyan
Write-Host ""
...
<!-- Botão Open Source Filter -->
    ...
    <Grid Margin="20">
        ...
        <Grid Grid.Row="0" Margin="0,0,0,16">
            ...
            <StackPanel Grid.Column="0" Orientation="Horizontal" VerticalAlignment="Center" Margin="0,0,16,0">
                ...
                <StackPanel VerticalAlignment="Center">
                    ...
                    <TextBlock Text="264 Aplicativos com Ícones Oficiais • Pack PC Gamer • Otimizações &amp; Tweaks" FontSize="12.5" Foreground="{DynamicResource TextSecondary}" Margin="0,3,0,0" />
                </StackPanel>
            </StackPanel>
        ...
        <TabControl Grid.Row="1" Background="Transparent" BorderThickness="0" Name="MainTabControl">
            ...
            <!-- ABA 1: APLICATIVOS -->
            <TabItem Name="TabItemApps" Header="📦 Aplicativos (264 Softwares)">
                <Grid Margin="0,16,0,0">
                    ...
                    <Grid Grid.Row="0" Margin="0,0,0,12">
                        ...
                        <!-- Resumo Contador -->
                        <TextBlock Name="TxtAppSummary" Grid.Column="2" VerticalAlignment="Center" Foreground="{DynamicResource TextSecondary}" FontSize="12" Text="Exibindo 264 de 264 aplicativos | 0 selecionados" />
                    </Grid>
...
function Update-SelectionSummary {
    ...
    if ($TabItemApps) {
        $TabItemApps.Header = if ($appCount -gt 0) { "📦 Aplicativos ($appCount selecionados)" } else { "📦 Aplicativos ($($Global:AppCatalog.Count) Softwares)" }
    }
    if ($TabItemTweaks) {
...
function Add-CustomWingetAppToQueue {
    ...
    # 1. Se já existir nos aplicativos padrão
...
"@
    ...
    $btnInstallNow.Add_Click({
        ...
        if ($selected) {
            ...
            try {
                $isMsStore = ($selected.Source -ieq "msstore") -or ($selected.Id -like "msstore:*")
                $pkgId = $selected.Id -replace "^msstore:\s*", ""
                $wingetArgs = if ($isMsStore) {
                    "/c winget install --id `"$pkgId`" -s msstore --accept-package-agreements --accept-source-agreements --disable-interactivity"
                } else {
...
$BtnPresetKitAndyz0x.Add_Click({
    ...
    Update-SelectionSummary
    Write-GuiLog "Softwares selecionados: Brave, VLC, ShareX, Notepad++, VS Code, Discord, Steam, Epic Games, Hydra Launcher, WhatsApp, Google Drive, SignalRGB, NodeJS LTS, Git, Python 3, PDF24, NanaZip, IDM, Nilesoft Shell, Proton Pass, Parsec, Snappy Driver, .NET Runtimes (6, 8, 9 e 10), AutoRuns, DirectX e Visual C++ Runtimes All-in-One." "INFO"
})
...
function Install-SelectedApps {
    ...
    foreach ($app in $selectedApps) {
        ...
        if ($app.DownloadUrl) {
            ...
            try {
                if (Test-Path $installerPath) {
                    if ((Get-Item $installerPath).Length -lt 100KB) {
        ...
        try {
            ...
            $pkgId = $app.Id -replace "^msstore:\s*", ""
            $wingetArgs = if ($isMsStore) {
                "/c winget install --id `"$pkgId`" -s msstore --accept-package-agreements --accept-source-agreements --disable-interactivity"
            } else {
                "/c winget install --id `"$pkgId`" -e --silent --accept-package-agreements --accept-source-agreements --disable-interactivity"
...
function Uninstall-SelectedApps {
    ...
    foreach ($app in $selectedApps) {
        ...
        try {
            $isMsStore = $app.Id -like "msstore:*"
            $pkgId = $app.Id -replace "^msstore:\s*", ""
            $wingetUninstallArgs = if ($isMsStore) {
                "/c winget uninstall --id `"$pkgId`" -s msstore --accept-source-agreements --disable-interactivity"
            } else {
                "/c winget uninstall --id `"$pkgId`" -e --silent --accept-source-agreements --disable-interactivity"
            }
            $process = Start-Process cmd.exe -ArgumentList $wingetUninstallArgs -NoNewWindow -PassThru -Wait
            ...
            if ($process.ExitCode -eq 0) {
                ...
                $process2 = Start-Process cmd.exe -ArgumentList "/c winget uninstall --name `"$($app.Name)`" -e --silent --accept-source-agreements --disable-interactivity" -NoNewWindow -PassThru -Wait
            ...
            <Border Grid.Row="1" Background="#0f111a" BorderBrush="#232838" BorderThickness="1" CornerRadius="10" Padding="16" Margin="0,0,0,20">
                <StackPanel VerticalAlignment="Center">
                    ...
                    <TextBlock TextWrapping="Wrap" FontSize="12" Foreground="#94a3b8" Margin="0,8,0,0">
                        ☕ Sua contribuição nos ajuda a manter a lista de mais de 264 softwares sempre atualizada, testar novas otimizações e continuar trazendo melhorias.
                    </TextBlock>
                </StackPanel>
            </Border>
...
"@
        ...
        Write-GuiLog "Aviso: Não foi possível exibir o diálogo de conclusão ($($_.Exception.Message))." "WARNING"
    }
}
